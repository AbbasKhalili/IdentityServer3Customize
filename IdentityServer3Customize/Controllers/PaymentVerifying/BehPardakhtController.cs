...
namespace JahanGostar.Controllers.PaymentVerifying
{
    ...
    public class BehPardakhtController : Controller
    {
        ...
        public BehPardakhtController(IIpgConfigoration ipgConfigoration, IFaildPaymentLog faildPaymentLog)
        {
        ...
        {
            var orderid = int.Parse(Request.Form["SaleOrderId"]);
            var token = Request.Form["refId"];
            var resultCode = Request.Form["ResCode"];
            var saleReferenceId = Request.Form["SaleReferenceId"];
            var amount = Request.Form["FinalAmount"];
            var cardHolderPan = Request.Form["CardHolderPan"];
            ...
            var verify = new VerifyResult
                ...
                OrderId = orderid,
                RefrenceNumber = saleReferenceId,
                CustomerCardNumber = cardHolderPan
            };
            try
            {
                var bPayment = new BPayment();
                var payModel = await bPayment.GetPayment(orderid);
                if (resultCode.Trim().ToLower() != "0")
                {
                    payModel.ReferenceNumber = saleReferenceId;
                ...
                if (resultCode == "0" && payModel.Status == 0)
                {
                    ...
                    refundparams.ReferenceNumber = saleReferenceId;
                    refundparams.SaleReferenceId = saleReferenceId;
                    ...
                    //verify.RefrenceNumber = saleReferenceId;
                    ...
                    var description = verify.Description + " شماره کارت " + verify.CustomerCardNumber + " رسید بانک " +
                                      verify.RefrenceNumber + " کد پیگیری " + verify.RefrenceNumber;
                    ...
                    paymentLog.AddationalData = verify.RefrenceNumber;
                    try
                    {
                        var done = await new BIncreaseCredit().FinalTransaction(orderid, (int)verify.Amount, verify.RefrenceNumber,
                            verify.Status, state, description, verify.RefrenceNumber);
